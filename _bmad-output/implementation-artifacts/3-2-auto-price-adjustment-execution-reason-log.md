...
- [ ] 실행 도메인과 run-key 가드를 추가한다. (AC: 1, 2, 3, 4)
  - [ ] `src/domain/pricing/auto-adjust-execution.ts`를 생성하고 due-time 평가, stale rule 감지, floor violation skip, duplicate-run result, recovery 상태를 모델링한다.
  - [ ] `src/domain/pricing/auto-adjust-execution.test.ts`에 실행 성공, floor skip, stale skip, 동시성 충돌, retry-after-partial-failure 케이스를 추가한다.
  - [ ] 규칙 입력/가격 경계는 Story 3.1의 `autoAdjustRule`와 `listingPricePolicy`를 재사용하고, 규칙 검증 로직을 중복 정의하지 않는다.

- [ ] producer event 계약과 발행 헬퍼를 추가한다. (AC: 1, 5)
  - [ ] `src/shared/contracts/events/pricing-auto-adjust-applied.v1.ts`를 생성하고 이벤트 envelope, payload, deterministic eventId 규약을 고정한다.
  - [ ] `src/shared/contracts/events/pricing-auto-adjust-applied.v1.test.ts`와 `tests/contracts/pricing-auto-adjust-applied.v1.contract.test.ts`에 canonical fixture, required field 검증, invalid payload rejection을 추가한다.
  - [ ] 성공 적용 이벤트에는 `eventId`, `occurredAt`, `traceId`, `schemaVersion`와 함께 listing/rule/run identifiers 및 before/after price, reason code를 포함한다.
  - [ ] skip 결과는 producer event 대신 execution record에 남기고, Epic 4가 소비할 관측 책임과 섞지 않는다.

- [ ] 실행 진입점과 durable guardrail을 연결한다. (AC: 1, 2, 3, 4, 5)
  - [ ] `src/app/api/pricing/auto-adjust/route.ts` 또는 동등한 scheduler-facing 진입점을 추가해 injected clock/runner로 실행을 오케스트레이션한다.
  - [ ] `src/infra/pricing/auto-adjust-execution.repository.ts` 또는 동등한 infra 저장소를 추가해 run-key idempotency와 applied-result persistence를 담당하게 한다.
  - [ ] page/component 계층은 infra를 직접 import하지 않도록 유지하고, mutation logic은 route/service 경계에만 둔다.

- [ ] 회귀 테스트를 추가해 single-apply semantics를 고정한다. (AC: 1, 2, 3, 4, 5)
  - [ ] floor-violation skip, stale rule skip, duplicate run, concurrent run, retry-after-partial-failure, happy-path apply를 각각 고정한다.
  - [ ] 라우트/통합 테스트로 producer helper가 기대한 schema를 발행하고, 동일 run key 재호출 시 중복 적용되지 않음을 확인한다.
  - [ ] Story 3.3의 history/minimal-signal 저장은 아직 구현하지 않는다.
