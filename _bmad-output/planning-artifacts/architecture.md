...
inputDocuments:
  ...
  - 'C:/Users/atima/Projects/PreProduct/_bmad-output/planning-artifacts/ux-design-specification-2026-04-07-revision.md'
  ...
  - 'C:/Users/atima/Projects/PreProduct/_bmad-output/planning-artifacts/research/market-preproduct-prelisting-mvp-research-2026-04-07.md'
...
- Event Contract (Active MVP canonical 4):
  - `listing.created.v1`
  - `ai.extraction.reviewed.v1`
  - `pricing.suggestion.accepted.v1`
  - `pricing.auto_adjust.applied.v1`
- Event 공통 규칙: `eventId`, `occurredAt(UTC)`, `traceId`, `schemaVersion`, dedupe 정책 적용
- FR25 RACI:
  - Epic 3: `pricing.auto_adjust.applied.v1` 발생 책임(도메인 실행 결과 발행)
  - Epic 4: 수집 관측/무결성 검증/경보 책임(운영 가드레일 및 스키마 거버넌스 소유)
  - 계약 소유: Epic 4가 스키마 버전 정책/하위호환 기준을 소유하고, Epic 3는 producer 준수 책임을 가진다.
...
- 프리리스팅 완료율: `>= 35%` (Week 3-8 공식 판정 윈도우)
- 7일 업데이트율: `>= 25%`
- 필수 필드 완성률(제목/카테고리/핵심 스펙 1+/가격): `>= 85%`
- 이벤트 중복률: `< 1%`
- 필수 이벤트 누락률: `< 2%`
- 판독 실패 수동입력 fallback E2E 통과율: `100%`
- 핵심 액션 p95(등록 시작/저장/수정): `<= 2s`
- FR25 경보 소유: Epic 4 운영 계층에서 위반 감지 시 Hold 규칙 연계
- Hold 적용 의미: 운영 판정(Hold/Stop) 상태 전환 규칙이며 사용자 핵심 플로우 차단 규칙이 아님
...
- Event Contract: 버전드 이벤트 네이밍/페이로드/호환 정책 + Active MVP canonical 4종 매핑 유지
- Errors: 표준 코드 + 복구 가이드
...
## CA Resolution Addendum (2026-04-07)

이 섹션은 IR(2026-04-07)에서 식별된 Major 이슈 3건을 아키텍처 결정으로 해소한다.

### CA-01: Authoritative Artifact Baseline 고정
...
- 규칙:
  - 구현/리뷰/QA 기준은 위 4개 문서를 우선한다.
  - 다른 버전 문서는 `Legacy Reference`로만 취급한다.
  - 충돌 시 `CA-01 baseline`이 우선한다.
- 근거: UX/에픽/아키텍처 참조선 불일치로 인한 구현 혼선을 차단하기 위함.
- 추적: `docs/architecture/adr/ADR-2026-04-07-01-authoritative-baseline.md`

### CA-02: UX Component Scope Alignment

- 상태: **Approved**
- 결정:
  - MVP 핵심 UX 컴포넌트 축은 revision UX 기준으로 고정한다.
  - MVP Core Components:
    - `PhotoUploader`
    - `ExtractionFieldEditor`
    - `PriceSuggestionCard`
    - `ListingSummarySubmitBar`
  - `DecisionCard`, `FitCriteriaPanel`, `ActionDecisionBar`는 `Deferred P1.5+`로 이동한다.
- 규칙:
  - Epic 4 스토리의 UX-DR/컴포넌트 참조는 위 MVP Core에 맞춰 재정렬한다.
  - P1.5+ 복귀 시 별도 ADR 업데이트 없이 활성화 금지.
- 근거: 선택 UX 기준선과 에픽의 컴포넌트 집합을 일치시켜 DoD 혼선을 제거.
- 추적: `docs/architecture/adr/ADR-2026-04-07-02-ux-component-alignment.md`

### CA-03: Active MVP vs Legacy Reference 실행 우선순위 강제

- 상태: **Approved**
- 결정:
  - 구현 범위 결정 시 아래 우선순위를 강제한다:
    1. `Active MVP`
    2. `Deferred P1.5+`
    3. `Legacy Reference`
  - `Legacy Reference`의 고복잡도 경계(Intent/Decision/Partner full model)는 MVP 구현 근거로 사용하지 않는다.
- 규칙:
  - 스토리 착수 전 스코프 태그(`Active MVP`/`Deferred`/`Legacy`)가 없으면 착수 금지.
  - PR 템플릿에 `Scope Tag` 체크를 필수화한다.
- 근거: 기존 고복잡도 경계와 revised-minimal MVP 경계 공존으로 인한 과구현 리스크 방지.
- 추적: `docs/architecture/adr/ADR-2026-04-07-03-scope-precedence.md`

### Implementation Follow-ups (Required)

1. `epics.md`에서 Epic 4의 UX 컴포넌트/UX-DR 매핑을 CA-02 기준으로 수정
2. 스토리 템플릿에 `Scope Tag` 필드 추가 (`Active MVP`/`Deferred`/`Legacy`)
3. PR 체크리스트에 `CA baseline conflict check` 항목 추가
